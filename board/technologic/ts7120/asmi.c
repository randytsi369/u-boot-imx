...
{
	writel(data, FPGA_ASMI);
	readl(FPGA_ASMI);
}

uint32_t static inline asmi_reg_read(void)
{
	return readl(FPGA_ASMI);
}
...
int asmi_read(uint8_t *buf, uint32_t addr, uint32_t size)
{
	int i;
...
static int do_asmi_write(unsigned int data, unsigned int offset, unsigned int len)
{
...
static int do_asmi(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if(strcmp(argv[1], "write") == 0) {
		...
		return do_asmi_write(data, offset, len);
	} else {
		ret = asmi_read((uint8_t *)data, offset, len);
	}

	return ret;
}

U_BOOT_CMD(asmi, 5, 0, do_asmi,
	"ASMI flash writing tool",
	" [read/write] [image address] [spiflash offset] [filesize]\n"
);
