...
int board_init(void)
{
	 ...
	 * we catch any of the pins low, then turn off the fet, drive the pins 
	 * low, wait, turn it back on, and this seems to fix the RTC issues. */
	gpio_direction_output(TS4900_ENRTC, 0);
	mdelay(2); // 2ms to turn on

	/* 5 is a magic number of retries.  The failure has never been observed
	 * past 2 retries. */
	for (i = 0; i < 5; i++)
	{
		...
		mdelay(1);
		gpio_direction_input(i2c_pad_info0.sda.gp);
		gpio_direction_input(i2c_pad_info0.scl.gp);
		if (gpio_get_value(TS4900_SCL) == 1 &&
			gpio_get_value(TS4900_SDA) == 1)
			break;

		/* needs at least 140ms to drain, and 2ms to turn on */
		gpio_direction_output(TS4900_ENRTC, 1);
		mdelay(200); 
		gpio_direction_output(TS4900_ENRTC, 0);
		mdelay(2);

		if(i == 4) puts ("WARNING: I2C Bus is being held low.\n");
	}
